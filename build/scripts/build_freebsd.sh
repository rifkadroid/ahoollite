...
if [ -z "${skip_kernel}" ]; then
	for kernel in ${KERNCONF:-Kontrol}; do
		...
		run "Building kernel (${kernel})" \
			"make -C ${srcdir} -s ${j} KERNCONF=${kernel} -m /usr/Kontrol/tmp/FreeBSD-src/contrib/llvm-project/clang/lib/Headers/ WERROR=-Wno-bitfield-constant-conversion buildkernel"
